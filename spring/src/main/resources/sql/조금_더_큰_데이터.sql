...
SELECT
    ...
    '더미 사용자 ' || gs,
    'user' || lpad(gs::text, 3, '0') || '@example.com',
    'pass1234',
    'USER',
...
END AS title,
    ...
    FROM generate_series(1, 5000) gs, u
...
-- =========================================================
-- 3) category_like 200000개 (기존 8000 -> 200000)
-- =========================================================
...
FROM generate_series(1, 200000), u, c
    )
...
-- =========================================================
-- 3.5) comment 더미데이터 (대용량)
-- - 루트 30000개 (기존 6000 -> 30000)
-- - 깊어질수록 확률/개수 낮춰서 총량 폭발 방지
-- =========================================================
...
SELECT
    ...
    (c.cids)[floor(random()*array_length(c.cids,1))::int + 1] AS category_id,
             ...
             ts,
             ts
FROM generate_series(1, 30000) gs, u, c,
    LATERAL (
    SELECT now() - (floor(random() * 365 * 24 * 60)::int * interval '1 minute') AS ts
    ) t
...
-- (2) depth=2: 루트당 0~3개 (60% 확률, 생성 시 1~3개)
WITH u AS (SELECT array_agg(id) AS uids FROM tmp_users),
     ins AS (
INSERT INTO "comment" (id, comment, parent_id, category_id, user_id, created_at, updated_at)
SELECT
...
FROM tmp_level1 p
    ...
    CROSS JOIN LATERAL (
    SELECT CASE
    WHEN random() < 0.60 THEN (floor(random()*3)::int + 1)
    ELSE 0
    END AS cnt
    ) r
...
-- (3) depth=3: depth2당 0~2개 (35% 확률, 생성 시 1~2개)
WITH u AS (SELECT array_agg(id) AS uids FROM tmp_users),
     ins AS (
INSERT INTO "comment" (id, comment, parent_id, category_id, user_id, created_at, updated_at)
SELECT
...
-- (4) depth=4: depth3당 0~1개 (15% 확률로 1개 생성)
WITH u AS (SELECT array_agg(id) AS uids FROM tmp_users),
     ins AS (
INSERT INTO "comment" (id, comment, parent_id, category_id, user_id, created_at, updated_at)
SELECT
...
-- (5) depth=5: depth4당 0~1개 (5% 확률로 1개 생성)
WITH u AS (SELECT array_agg(id) AS uids FROM tmp_users),
     ins AS (
INSERT INTO "comment" (id, comment, parent_id, category_id, user_id, created_at, updated_at)
SELECT
...
-- =========================================================
-- 4) check_record: CHECK 카테고리당 최근 365일 (기존 120일)
-- =========================================================
...
-- =========================================================
-- 5) check_list_record: CHECKLIST 카테고리당 최근 180일 * 하루 6개 (기존 60일 * 4개)
-- =========================================================
...
-- =========================================================
-- 6) text_record: TEXT 카테고리당 최근 180일 (기존 90일)
-- =========================================================
...
-- =========================================================
-- 7) time_record: TIME 카테고리당 최근 180일 (기존 90일)
-- =========================================================
...
-- =========================================================
-- 8) number_record: NUMBER 카테고리당 최근 365일 (기존 120일)
-- =========================================================
